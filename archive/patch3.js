...
// In newCode, get_registration is followed by get_reg_config
newCode = newCode.replace(extract(newCode, 'if (action === "get_registration") {', 'if (action === "get_reg_config") {'), getRegOld);

// 4. get_shift_registrations
const getShiftRegOld = extract(oldCode, 'if (action === "get_shift_registrations") {', 'if (action === "sync_roster") {');
// In newCode, get_shift_registrations is followed by sync_roster or // Pre-flight
