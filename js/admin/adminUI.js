        ...
        const posCells = shift
          ? shift.colHeaders
              .map((_, idx) => {
                ...
                return `<td><span class="position-tag ${!p ? "empty" : ""}" style="cursor: pointer;" onclick="AdminApp.editPosition('${emp.id}', ${idx}, '${p || ""}')" title="Nhấn để cập nhật Vị trí làm việc">${p || "Chưa xếp"}</span></td>`;
              })
              .join("")
          : "";
